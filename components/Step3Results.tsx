...
import { 
  ...
  Globe, ChevronUp, ExternalLink, Info, XCircle, AlertCircle
} from 'lucide-react';
...
const Step3Results: React.FC<Props> = ({ brandInfo, queries, docId, onReset, initialResults }) => {
  ...
  const fetchQueryAnalysis = async (query: string, idx: number) => {
    // If we have preloaded data for this index, don't fetch
  ...
  useEffect(() => {
    // Only fetch if initialResults wasn't provided
    if (!initialResults) {
      queries.forEach((query, idx) => {
        fetchQueryAnalysis(query, idx);
      });
    }
  }, [brandInfo, queries, docId, initialResults]);

  const metrics = useMemo(() => {
    const items = Object.values(results) as AnalysisItem[];
    ...
    const visibilityPercentage = Math.round(totalScore / totalCount);
    
    return {
      visibility: `${visibleCount}/${totalCount}`,
      avgRank: avgRank,
      score: `${visibilityPercentage}%`,
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard 
          ...
          loading={!results[0] && loadingStates[0]}
        ...
        <MetricCard 
          ...
          value={metrics.avgRank !== 'N/A' ? `#${metrics.avgRank}` : 'N/A'}
          loading={!results[0] && loadingStates[0]}
        ...
        <MetricCard 
          ...
          accentColor="bg-orange-50 text-orange-400"
        />
      </div>

      <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-2xl shadow-indigo-100/40">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-3xl font-black text-gray-900">Detailed Query Analysis</h2>
              <p className="text-gray-500 font-medium">Breakdown of brand visibility for each search query.</p>
            </div>
          </div>
          <button 
            onClick={onReset}
            className="flex items-center justify-center gap-2 px-6 py-3 text-sm font-black text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-all border-2 border-indigo-100"
          >
        ...
        <div className="space-y-4">
          {queries.map((query, idx) => (
            <AnalysisItemRow 
              ...
              error={errorStates[idx]}
              docId={docId}
              brandDomain={brandInfo.domain}
              onRetry={() => fetchQueryAnalysis(query, idx)}
            />
          ))}
        </div>
      </div>
...
const MetricCard: React.FC<{ 
  ...
  shadowColor: string;
  accentColor: string;
}> = ({ icon, label, value, loading, borderColor, shadowColor, accentColor }) => (
  <div className={`bg-white rounded-2xl p-8 border-l-4 ${borderColor} ${shadowColor} shadow-xl flex items-center gap-6 transition-all hover:translate-y-[-2px]`}>
    <div className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 ${accentColor}`}>
      {icon}
    </div>
    <div className="space-y-1">
      ...
      <p className="text-xs font-bold text-gray-400 leading-tight uppercase tracking-wider">{label}</p>
    </div>
  </div>
);

const AnalysisItemRow: React.FC<{ 
  query: string; 
  data?: AnalysisItem; 
  loading: boolean; 
  error?: boolean;
  ...
  brandDomain: string;
  onRetry: () => void;
}> = ({ query, data, loading, error, index, docId, brandDomain, onRetry }) => {
  ...
  const renderStatusIcon = () => {
    if (loading) return <Loader2 className="w-6 h-6 text-indigo-600 animate-spin" />;
    if (error) return <AlertCircle className="w-6 h-6 text-amber-500" />;
    if (data) {
  ...
  return (
    <div className="border border-gray-100 rounded-2xl overflow-hidden transition-all bg-white hover:border-indigo-200 shadow-sm">
      <div 
        className={`p-6 flex items-center justify-between cursor-pointer transition-colors ${expanded ? 'bg-indigo-50/20' : 'hover:bg-gray-50/50'}`}
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex items-center gap-4 flex-grow">
          {renderStatusIcon()}
          <span className={`text-lg font-bold ${loading ? 'text-gray-400' : 'text-gray-700'} line-clamp-1`}>
            "{query}"
          </span>
        </div>
        <ChevronDown className={`w-6 h-6 text-gray-400 transition-transform duration-300 ${expanded ? 'rotate-180 text-indigo-600' : ''}`} />
      </div>
          ...
          {loading ? (
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="relative bg-indigo-50/20 p-8 rounded-3xl border border-indigo-50 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                {recs.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {recs.map((rec, i) => (
                      <div key={i} className="bg-amber-50/40 p-6 rounded-2xl border border-amber-100 text-amber-900 font-medium text-xs flex gap-4 shadow-sm group hover:bg-amber-50 transition-colors leading-relaxed">
                  ...
                  <div className="flex flex-col items-center justify-center p-12 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200">
                    <p className="text-gray-400 font-bold text-sm mb-6">No recommendations available for this query.</p>
                    <button 
                      onClick={handleGenerateRecs}
                      disabled={isGeneratingRecs}
                      className="flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:bg-gray-400 transition-all transform hover:-translate-y-0.5 active:scale-95"
                    >
                  ...
                  {data.sources && data.sources.length > 0 ? data.sources.map((source, i) => {
                    ...
                    return (
                      <a 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black tracking-wider uppercase border transition-all ${isMatched ? 'bg-indigo-600 text-white border-indigo-700 shadow-md shadow-indigo-100' : 'bg-gray-100 text-gray-500 border-gray-200 hover:bg-gray-200'}`}
                      >
...
const StatusBox: React.FC<{ icon: React.ReactNode, label: string, value: string, success?: boolean, bold?: boolean }> = ({ icon, label, value, success, bold }) => (
  <div className="bg-gray-50/50 p-5 rounded-2xl border border-gray-100 flex items-center gap-5 shadow-sm hover:border-indigo-100 transition-colors">
    <div className="text-indigo-400 bg-white p-2.5 rounded-xl shadow-sm border border-gray-100">{icon}</div>
    <div className="flex flex-col">
      <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1.5">{label}</span>
      <div className="flex items-center gap-2">
        <span className={`text-sm ${bold ? 'font-black text-gray-900' : 'font-bold text-gray-700'}`}>{value}</span>
        {success !== undefined && (
          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${success ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}>
            {success ? 'Yes' : 'No'}
          </span>
        )}
      </div>
