...
import { 
  ...
  Globe, ChevronUp, ExternalLink, Info, XCircle, AlertCircle, Clock, Mail
} from 'lucide-react';
...
const Step3Results: React.FC<Props> = ({ brandInfo, queries, docId, onReset, initialResults }) => {
  ...
  const fetchQueryAnalysis = async (query: string, idx: number) => {
    // Double check if we already have results for this index to avoid unnecessary calls
  ...
  useEffect(() => {
    // Only run if initialResults wasn't provided AND we haven't started analysis yet
    ...
    <div className="space-y-12 animate-in fade-in duration-700 px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard 
          ...
          loading={loadingStates[0] && !results[0]}
...
const AnalysisItemRow: React.FC<{ 
  ...
  brandDomain: string;
  onRetry: () => void;
}> = ({ query, data, loading, isWaiting, error, index, docId, brandDomain, onRetry }) => {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all bg-white ${isWaiting ? 'opacity-50 border-gray-100 grayscale hover:grayscale-0 transition-all cursor-not-allowed' : 'border-gray-100 hover:border-indigo-200 shadow-sm'}`}>
      <div 
